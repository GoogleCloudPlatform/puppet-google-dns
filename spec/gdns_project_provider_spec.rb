...
describe Puppet::Type.type(:gdns_project).provider(:google) do
  before(:all) do
  ...
  context '#flush' do
    subject do
      Puppet::Type.type(:gdns_project).new(
        name: 'my-name'
      ).provider
    end
    context 'no-op' do
  ...
  def create_type(id)
    Puppet::Type.type(:gdns_project).new(
      ...
      name: P_NAME_DATA[(id - 1) % P_NAME_DATA.size]
    )
  end

  def expand_variables(template, data, extra_data = {})
    Puppet::Type.type(:gdns_project).provider(:google)
