...
describe Puppet::Type.type(:gdns_resource_record_set).provider(:google) do
  ...
  RRS_MANAGED_ZONE_DATA = %w[
    resource(managed_zone,0)
    resource(managed_zone,1)
    resource(managed_zone,2)
    resource(managed_zone,3)
    resource(managed_zone,4)
  ].freeze
  ...
  def expect_network_get_success_managed_zone(id, data = {})
    ...
    expect(Google::Dns::Network::Get).to receive(:new)
      .with(self_link_managed_zone(uri_data(id).merge(data)),
            instance_of(Google::FakeAuthorization)) do |args|
      debug ">> GET #{args}"
      request
    end
  ...
  def create_type(id)
    Puppet::Type.type(:gdns_resource_record_set).new(
      ...
      project: RRS_PROJECT_DATA[(id - 1) % RRS_PROJECT_DATA.size],
      managed_zone: RRS_MANAGED_ZONE_DATA[(id - 1) \
        % RRS_MANAGED_ZONE_DATA.size],
      name: RRS_NAME_DATA[(id - 1) % RRS_NAME_DATA.size],
      type: RRS_TYPE_DATA[(id - 1) % RRS_TYPE_DATA.size]
    )
  end
