...
require 'google/dns/property/resourceref'
...
Puppet::Type.type(:gdns_resource_record_set).provide(:google) do
  ...
  def self.fetch_to_hash(fetch)
    {
      name: Google::Dns::Property::String.parse(fetch['name']),
      type: Google::Dns::Property::Enum.parse(fetch['type']),
      ttl: Google::Dns::Property::Integer.parse(fetch['ttl']),
      target: Google::Dns::Property::StringArray.parse(fetch['rrdatas'])
    }.reject { |_, v| v.nil? }
  end
  ...
  def self.resource_to_hash(resource)
    managed_zone = Puppet::Type.type(:gdns_managed_zone)
                               .fetch(resource[:managed_zone])
                               .exports[:name]
    {
      project: resource[:project],
      name: resource[:name],
  ...
  def self.return_if_object(response, kind)
    ...
    raise "Incorrect result: #{result['kind']}" \
      unless result['kind'] == 'dns#resourceRecordSetsListResponse'
    return_if_extract_object(result, kind)
  end
  ...
  def self.return_if_extract_object(result, kind)
    return if result['rrsets'].empty?
    result = result['rrsets'][0]
    raise "Incorrect result: #{result['kind']}" \
  ...
  def prefetch_soa_resource
    ...
    result = fetch_resource(resource, self_link(resource),
                            'dns#resourceRecordSet')
    if result.nil?
